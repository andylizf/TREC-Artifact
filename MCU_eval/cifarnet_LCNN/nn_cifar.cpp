...
float32_t run_nn(q7_t* input_data, q7_t* output_data, q7_t* LSH_wt, uint8_t options) {
 ...
 //t.stop();
   ...
   * 
   **/
  int dictionary_size = 50, s = 3, lcnn_index = 0;
  for (int i  = 0; i < CONV2_IN_CH; i ++)
    for (int j = 0; j < s; j ++)
      for (int k = 0; k < CONV2_KER_DIM; k ++)
        for (int l = 0; l < CONV2_KER_DIM; l ++)
          col_buffer[lcnn_index ++] = rand() % dictionary_size;
  t.start();
  
  arm_convolve_HWC_q7_LCNN(buffer2, CONV2_IN_DIM, CONV2_IN_CH, dictionary_size, s, conv2_wt, col_buffer, conv2_wt, CONV2_OUT_CH, CONV2_KER_DIM, CONV2_PAD, CONV2_STRIDE, 0, CONV2_OUT_RSHIFT, buffer1, CONV2_OUT_DIM);
