 ...
 * Title:        arm_convolve_HWC_q7_LCNN.c
 * Description:  Fast Q7 version of lookup-based convolution
 *
...
arm_status arm_convolve_HWC_q7_LCNN(const q7_t *Im_in,
                                    const uint16_t dim_im_in,
                                    const uint16_t ch_im_in,
                                    const uint16_t dictionary_size,
                                    const uint16_t s,
                                    const q7_t *S,
                                    const q7_t *I,
                                    const q7_t *C,
                                    ...
                                    const uint16_t coefficient_shift,
                                    ...
                                    const uint16_t dim_im_out)
{
    ...
    /**
     * S: k*w*h
     * I: ch_im_out * s * dim_kernel * dim_kernel
     * C: ch_im_out * s * dim_kernel * dim_kernel
     **/ 
    for (int i_out_x = 0; i_out_x < dim_im_out; i_out_x ++) for (int i_out_y = 0; i_out_y < dim_im_out; i_out_y ++) {
        
        int result = 0;

        for (int filter_index = 0; filter_index < ch_im_out; filter_index ++) {
            for (int kernel_offset_x = 0; kernel_offset_x < dim_kernel; kernel_offset_x ++)
                for (int kernel_offset_y = 0; kernel_offset_y < dim_kernel; kernel_offset_y ++) for (int t = 0; t < s; t ++) {
                    if (i_out_x - padding + kernel_offset_x < 0 || i_out_x - padding + kernel_offset_x >= dim_im_out 
                     || i_out_y - padding + kernel_offset_y < 0 || i_out_y - padding + kernel_offset_y >= dim_im_out)
                        continue;
                    
                    //int index = I[filter_index][t][kernel_offset_x][kernel_offset_y];
                    q7_t index = *( I + ((filter_index * s + t) * dim_kernel + kernel_offset_x ) * dim_kernel + kernel_offset_y);

                    //result += C[filter_index][t][kernel_offset_x][kernel_offset_y]
                    //        * S[ I[filter_index][t][kernel_offset_x][kernel_offset_y] ][i_out_x - padding + kernel_offset_x][i_out_y - padding + kernel_offset_y];



                    result +=
                        *(C + (((filter_index * s + t) * dim_kernel) + kernel_offset_x) * dim_kernel + kernel_offset_y) *
                        *(S + (((index * dim_im_out + (i_out_x - padding + kernel_offset_x) ) * dim_im_out + (i_out_y - padding + kernel_offset_y) )));

                }
        }
        *(Im_out + i_out_x * dim_im_out + i_out_y) = (q7_t)__SSAT((result >> out_shift), 8);
    }
