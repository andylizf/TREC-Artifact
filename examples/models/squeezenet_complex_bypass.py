...
class SqueezeNet_TREC(nn.Module):
    def __init__(self, params_L, params_H, bp_params_L, bp_params_H, trec=[0]*(8*3+2), bypass_trec=[0]*4, class_num=10):
        ...
        if bypass_trec[0]:
            self.bp_conv1 = Conv2d_TREC(
                96, 128, 1, bp_params_L[0], bp_params_H[0], 0)
        else:
            self.bp_conv1 = nn.Conv2d(96, 128, 1)

        if bypass_trec[1]:
            self.bp_conv2 = Conv2d_TREC(
                128, 256, 1, bp_params_L[1], bp_params_H[1], 1)
        else:
            self.bp_conv2 = nn.Conv2d(128, 256, 1)

        if bypass_trec[2]:
            self.bp_conv3 = Conv2d_TREC(
                256, 384, 1, bp_params_L[2], bp_params_H[2], 2)
        else:
            self.bp_conv3 = nn.Conv2d(256, 384, 1)

        if bypass_trec[3]:
            self.bp_conv4 = Conv2d_TREC(
                384, 512, 1, bp_params_L[3], bp_params_H[3], 3)
        else:
