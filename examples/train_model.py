...
def test(net, testset, testloader): 
    ...
    logging.info('Accuracy of the network on the 10000 test images: {}'.format(correct / total))
...
def main():
    ...
    scheduler = optim.lr_scheduler.StepLR(optimizer, args.step, gamma=0.1, last_epoch=-1)
    for epoch in range(args.epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            optimizer.step()
            if i % 100 == 99:
                logging.info('[epoch=%d, batch=%5d] loss: %.3f', epoch, i + 1, running_loss / 100)
                running_loss = 0.0
        scheduler.step()
        ...
        if epoch == 49:
            ...
            utils.save(net, os.path.join(args.checkpoint_path, 'weights_50.pt'))
