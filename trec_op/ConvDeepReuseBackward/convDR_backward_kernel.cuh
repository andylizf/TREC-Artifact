...
#define DEBUG_INFO cudaDeviceSynchronize(); \
...
__global__ void reconstruct_gradInputRows_cuda_kernel(
    ...
    CUDA_1D_KERNEL_LOOP(global_id, num_rows * n_matrices) {
        ...
        scalar_t *row_start = gradInput_rows + 
                                (matrix_offset * n_matrices + matrix_id) * param_L;
        const scalar_t *buck_start = gradInput_centroids + 
                                    (matrix_id * max_buckets + vect_index) * param_L;
    ...
    CUDA_1D_KERNEL_LOOP(index, n) {
        ...
        for (int64_t h_row = h_row_start; h_row < h_row_end; h_row += 1) {
            for (int64_t w_row = w_row_start; w_row < w_row_end; w_row += 1) {
                ...
                int64_t data_row_index = ((((batch_id * height_row + h_row) * width_row + 
                                            w_row) * channels + c_im) * kernel_height + 
                                            h_k) * kernel_width + w_k;  
