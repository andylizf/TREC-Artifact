...
__global__ void im2row_DRbatch_cuda_kernel(
    ...
    CUDA_1D_KERNEL_LOOP(index, n) { // 表示线程数大于当前grid开启上限时，一直在block中循环线程计算直到完成任务。
...
__global__ void count_buckets_cuda_kernel(
    ...
    uint64_t tid = threadIdx.x;
    uint64_t i = blockIdx.x * blockDim.x + tid;
    if(i >= total_buckets * num_rows)  return;
...
__global__ void index_bucket_cuda_kernel(
    ...
    int* buckets_stats) {         // sum and max of buckets_num
...
void get_id_count_cuda(
    ...
    int64_t num_vectors = hashed_vectors.size(0);   // n_matrices * num_rows
    int64_t vector_len = hashed_vectors.size(1);    // H
    int64_t n_matrices = vector_ids.size(0);
    int64_t num_rows = vector_ids.size(1); // [n_matrices, num_rows]
...
void get_vector_index_cuda(
    ...
    get_vector_index_cuda_kernel
    <<<GET_BLOCKS(n_matrices * num_rows), CUDA_NUM_THREADS, 0, stream>>>(
        n_matrices,
        num_rows,
        total_buckets,
        vector_ids.data_ptr<ID_DATATYPE>(),
...
void get_buckets_count_out_cuda(
    ...
    get_buckets_count_out_cuda_kernel
    <<<GET_BLOCKS(n_matrices * total_buckets), CUDA_NUM_THREADS, 0, stream>>>(
        n_matrices,
        total_buckets,
        max_buckets,
        buckets_index.data_ptr<int>(),
